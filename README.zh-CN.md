...
这个 skill 输出 HTML 格式的 Review Preflight 报告。报告不是固定模板，而是根据 PR 内容选择更合适的展示方式。
...
HTML 可以使用更丰富的表达方式，例如指标条、review 路线时间线、风险矩阵、卡片、表格、提示区和 checkbox 列表。原则是帮助 reviewer 更快扫描和判断，而不是做装饰。
...
输出应只包含 HTML。默认输出 HTML fragment，并使用清晰的根容器；如果用户要求独立页面或视觉预览，可以输出完整 HTML 文档和内联 CSS。
