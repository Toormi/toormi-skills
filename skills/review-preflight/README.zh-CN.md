...
Review Preflight 是一个 Agent skill，用来把 Pull Request 转换成面向人工 reviewer 的自包含 HTML 预检报告文件。它帮助 reviewer 在打开完整 diff 之前，先理解 PR 的意图、核心变更、变更形状、推荐 review 路线和需要重点怀疑的风险区域。
...
HTML 可以使用更丰富的表达方式，例如指标条、review 路线时间线、风险矩阵、带注释的 diff 片段、Before / After 变更图、模块关系图、卡片、表格、提示区、tabs、折叠区和 checkbox 列表。原则是帮助 reviewer 更快扫描和判断，而不是做装饰。

## 可视化变更图

当 PR 用图更容易 review 时，Review Preflight 可以直接在 HTML 报告里包含图：

| PR 改动类型 | 图形 |
| --- | --- |
| 模块、文件、组件、依赖关系变化 | 变更影响图或依赖图 |
| 函数调用或执行路径变化 | 调用图 |
| 业务规则、条件判断、错误处理、重试降级变化 | 流程图 |
| API、服务、队列、webhook 或第三方交互变化 | 时序图 |
| 类、接口、字段、方法、继承关系变化 | 类图 |
| 生命周期或状态流转变化 | 状态图 |
| 表、字段、migration、实体关系变化 | ER / 数据模型图 |

图应该保持小而清晰，使用真实代码名称，标记新增、删除、修改的节点或连接，并聚焦人工 reviewer 最应该先检查的路径。
...
Review Preflight 有四层价值：

1. 理解层：帮助 reviewer 知道这个 PR 是什么。
2. 可视化层：帮助 reviewer 看清变更形状。
3. 导航层：帮助 reviewer 决定先看哪里。
4. 怀疑层：帮助 reviewer 知道哪里需要验证。

真正的价值在可视化层、导航层和怀疑层。普通 PR summary 很容易变成“看起来有用但没人读”的内容；变更图、review 路线和风险预检可以直接改变 reviewer 的注意力分配。
